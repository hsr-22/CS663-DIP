...
function [tp_count, tn_count, fp_count, fn_count] = compute_confusion_matrix(eigen_coef, test_coef, fake_coef, threshold)
    ...
    for j = 1:n_test
        [m, ~] = min(sum((eigen_coef - test_coef(:, j)).^2));
        if (m < threshold)
            tn_count = tn_count + 1;
        else
            fp_count = fp_count + 1;
...
threshold_range = 70:1:200;

% Loop over different threshold values
for threshold = threshold_range
    % Get the confusion matrix counts using the function
    [tp_count, tn_count, fp_count, fn_count] = compute_confusion_matrix(eigen_coef, test_coef, fake_coef, threshold);
